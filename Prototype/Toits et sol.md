...
| | Ce que c'est |
|---|---|
| ① le masque | une petite vue à part, invisible, où **l'îlot choisi est redessiné seul**, en blanc plat sur du vide, avec la **même caméra** que l'image |
...
rectangle caché — le contour ne consomme rien du tout. Rien n'a été ajouté à
l'export : `07` avait dû exporter les anneaux pour le ruban, il ne les exporte
plus.
...
- ☐ **Le stationnement** : `routes.places` compte 4 587 places à Wehrau et
  **aucune ne se voit**. C'est le sujet du jeu, et c'est la plus grosse chose
  que le sol ne dit pas encore. 🔄 Le **marquage**, lui, est fait depuis le
  2026-08-18 (§ 3 ter) ; la place du stationnement reste **réservée dans
  l'image** — les mètres libres entre la bordure et l'asphalte sont exactement
  là où il se dessinera, et il aura la même forme qu'ici : des règles qui
  lisent la largeur, pas une liste de places.
