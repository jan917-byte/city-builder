...
**2026-08-25 (session 63) — les messages de commit deviennent une règle écrite.** Le style imagé des titres (« La percée se prend sur la façade… ») n'avait **jamais été décidé** : il apparaît au 3ᵉ commit du dépôt, dans une session avec moi, et chaque session suivante le recopiait en lisant le `git log`. Une boucle, pas une intention.
Il est remplacé par **titre factuel préfixé par zone + une ligne de contexte** (`CLAUDE.md` §6) : le `git log` redevient cherchable — un titre imagé ne dit ni quel dossier a bougé, ni ce qui a été mesuré. Les anciens messages restent tels quels, on ne réécrit pas l'historique.

**2026-08-25 (session 62) — l'îlot compact n'est plus un anneau fermé.** Demandé devant l'image : « un entre-deux, cour intérieure accessible, compact mais pas continu sur toute la longueur ». `04d` ouvre une **percée** de 9 m dès que le mur mitoyen dépasse 60 m — décidée par ÎLOT et non par parcelle, parce qu'un mur continu est une somme de façades voisines : sur l'îlot 49, dix bâtiments faisaient 286 m d'un seul tenant sans qu'aucun ne dépasse 31 m.
🔴 **La percée est une tranche de FAÇADE retirée à la PARCELLE, pas une entaille dans le bâtiment**, et c'est tout l'écart entre un pignon et un défaut. Creusée dans l'empreinte finie, elle mordait n'importe où — la 559 sortait en coin taillé, la 558 en C. Prise sur le trottoir avant que `bande_sur_rue` ne dessine, elle laisse un mur droit perpendiculaire à sa rue, et le reste de la tranche ne coûte rien puisqu'il n'y a que de la cour derrière. Trois autres pièges payés : R2 bis annulait la percée en silence et il lui faut son exception · un retrait latéral est une droite INFINIE (559 : 258 → 48 m²) · deux voisines partagent aussi leur FOND, qu'ouvrir creuse une tranchée sans rien ouvrir.
Îlot 49 : mur le plus long **85 → 58 m**, cinq percées de 9 m, 96 → 81 % du tour bâti, et **toutes les empreintes plus régulières qu'avant** (remplissage 0,61 → 0,82 sur la 554, 0,61 → 0,78 sur la 559 ; aucun angle sous 70°). Les deux nombres (60 m, 9 m) sont du **level design** et attendent l'auteur.
